...
Escopo: só os mercados das views ATIVAS (2.2 inflação, 2.3 Fed reunião, B
trajetória do Fed), que já estão baixados em data/raw/clob_exploracao/:
  - 2.2: famílias CPI_* (todos os meses) e M1_cpi_monthly_*
  - 2.3: M2_fomc_*
  - B  : M3_fed_trajectory_* (as faixas de cortes em 2025)
...
def main():
    ...
    print(f"Nº de mercados:       {len(files)}  "
          f"(2.2={sum(1 for f in files if scope_view(f)=='2.2')}, "
          f"2.3={sum(1 for f in files if scope_view(f)=='2.3')}, "
