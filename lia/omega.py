...
A forma funcional exata (Decisão pendente 4, ver Decisoes_pendentes.md)
...
class SinaisOmega:
    ...
    volume_liquidez: float  # TODO(DECISAO-4): unidade/normalização a definir
    estabilidade_probabilidade: float  # TODO(DECISAO-4): unidade/normalização a definir
    convergencia_fontes: float  # TODO(DECISAO-5): forma de medir conflito entre fontes
    proximidade_evento: float  # TODO(DECISAO-4): unidade (dias? fração da janela?)


def calcular_omega(sinais: SinaisOmega) -> float:
    """Calcula a confiança reativa (Ω) a partir dos sinais de mercado.

    TODO(DECISAO-4): forma funcional (linear, não-linear, pesos, faixa de
    saída) ainda não decidida — ver Decisoes_pendentes.md.
    """
    raise NotImplementedError(
        "Forma funcional do Ω reativo ainda não decidida "
        "(ver Decisoes_pendentes.md, Decisão 4)"
    )
